...
public class AudioManager : MonoBehaviour
{
    ...
    public void Play3dOneShotSound(AudioClip _clip, string mixerGroup, float _radius, Vector3 _pos, float _volume = 1, float _minPitch = 0.75f, float _maxPitch = 1.25f)
    {
        AudioSource _as = GetUnused3dAS();
        _as.minDistance = _radius;
        _as.maxDistance = _radius * 5;
        _as.gameObject.transform.position = _pos;
        _as.spatialBlend = 1;
        PlayOneShotSound(_as, _clip, mixerGroup, _minPitch, _maxPitch, _volume);
    }
